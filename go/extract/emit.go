...
func emitChoiceBody(buf *strings.Builder, si StructInfo, rules map[string]RuleInfo) {
	...
	fmt.Fprintf(buf, "\t}\n\n")
	fmt.Fprintf(buf, "\tchildType := t.Type(child)\n")
	fmt.Fprintf(buf, "\tchildName := t.Name(child)\n\n")
	...
	for _, f := range si.Fields {
		...
		if innerType == "string" {
			fmt.Fprintf(buf, "\tcase childType == NodeType_Node && childName == %q:\n", f.LLTag)
	...
	fmt.Fprintf(buf, "\tcase childType == NodeType_String:\n")
...
func emitSequenceBody(buf *strings.Builder, si StructInfo, rules map[string]RuleInfo) {
	// Walk children, matching named nodes by name
	...
	fmt.Fprintf(buf, "\t\t\treturn true\n")
	fmt.Fprintf(buf, "\t\t}\n")
	fmt.Fprintf(buf, "\t\tswitch t.Name(cid) {\n")

...
// BenchmarkExtractOnlyArenaNoCount measures arena extraction without the
// pre-count pass — arenas are pre-sized once outside the loop.
func BenchmarkExtractOnlyArenaNoCount(b *testing.B) {
	inputs := benchInputs(b)
	p := NewJSONParser()
	p.SetShowFails(false)
	...
	for _, name := range inputNames {
		...
		// Pre-count once outside the loop.
